...
void star_d0_bw_0_80(const int scan = 0)
{
  TF1 *funBW = new TF1("funBW", StaticBGdNdPtD0, 0.2, 5., 3);
  funBW->FixParameter(0,0.497172);
  funBW->FixParameter(1,0.218378);
  funBW->FixParameter(2,2437.050455);
  ...
  htmp = new TH1D("htmp","",1,xmin, xmax);
  ...
  htmp->Draw();
   ...
   gr0->SetPoint(0,0.314591,0.0913340);
   gr0->SetPointError(0,0,0.0160695);
   gr0->SetPoint(1,0.967794,0.0730444);
   gr0->SetPointError(1,0,0.0127212);
   gr0->SetPoint(2,1.35482,0.0470035);
   gr0->SetPointError(2,0,0.00643744);
   gr0->SetPoint(3,1.88638,0.0161183);
   gr0->SetPointError(3,0,0.00251520);
   gr0->SetPoint(4,2.56187,0.00301753);
   gr0->SetPointError(4,0,0.000635715);
   gr0->SetPoint(5,3.74846,0.000307001);
   gr0->SetPointError(5,0,0.0000555378);
  ...
  tx.DrawLatex(txx,0.84,"0-80% Au+Au 200 GeV");
  char txbeta[100];
  sprintf(txbeta,"<#beta^{1}_{T}> = %4.2f #pm %4.2f",funBW->GetParameter(0)*2./3.,0.09);
  tx.DrawLatex(txx,0.8,txbeta);
  char txtemp[100];
  sprintf(txtemp,"T^{1} = %d #pm %d MeV",funBW->GetParameter(1)*1000,75.);
