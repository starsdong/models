...
void plotInvYieldRatio(const char *ver = "RHIC")
{
  gROOT->Reset();
  ...
  double rr1[50][100], rr1e[50][100];
  double rr2[50][100], rr2e[50][100];
  for(int i=0;i<50;i++) {
    if(i!=0) {
      for(int j=0;j<100;j++) {
	double rA = yy1[i][10]/yy1[7][10];
	rr1[i][j] = yy1[7][j]!=0 ? yy1[i][j]/yy1[7][j]/rA : 0.0;
	rr1e[i][j] = (yy1[7][j]!=0 && yy1[i][j]!=0) ? rr1[i][j]*sqrt(pow(yy1e[i][j]/yy1[i][j],2.)+pow(yy1e[7][j]/yy1[7][j],2.)) : 0.0;
	
	double rB = yy2[i][10]/yy2[7][10];
	rr2[i][j] = yy2[7][j]!=0 ? yy2[i][j]/yy2[7][j]/rB : 0.0;
	rr2e[i][j] = (yy2[7][j]!=0 && yy2[i][j]!=0) ? rr2[i][j]*sqrt(pow(yy2e[i][j]/yy2[i][j],2.)+pow(yy2e[7][j]/yy2[7][j],2.)) : 0.0;
      }
    }

//     gr1[i] = new TGraphErrors(100,mT,yy1[i],0,yy1e[i]);
//     gr2[i] = new TGraphErrors(100,mT,yy2[i],0,yy2e[i]);
    gr1[i] = new TGraphErrors(100,mT,rr1[i],0,rr1e[i]);
    gr2[i] = new TGraphErrors(100,mT,rr2[i],0,rr2e[i]);
  }
...
//       Terr[i] = fun1->GetParError(1);
      ...
      TLatex *tex = new TLatex(1.5, 5, title[i]);
      tex->SetTextFont(42);
      ...
      TLatex *tex = new TLatex(0.5, 5., text);
      tex->SetTextFont(42);
...
//   }
  ...
  TLatex *tex = new TLatex(XMIN-0.08, YMAX/2, "Ratio");
  tex->SetTextFont(42);
  tex->SetTextSize(0.05);
  tex->SetTextAngle(90);
  tex->Draw("same");
  ...
  c1->Update();
  char outfig[100];
  sprintf(outfig,"fig/invYieldRatio_%s.eps",ver);
  c1->SaveAs(outfig);
  sprintf(outfig,"fig/invYieldRatio_%s.gif",ver);
  c1->SaveAs(outfig);
